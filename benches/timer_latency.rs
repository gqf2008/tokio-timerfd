...
async fn measure_delay_queue(period: Duration, entries: u64) -> Vec<i128> {
    let mut queue = DelayQueue::new().expect("failed to create DelayQueue");
    let start = Instant::now() + period;
    let mut samples = Vec::with_capacity(entries as usize);

    for index in 0..entries {
        let deadline = start + period * (index as u32);
        queue.insert_at(index, deadline);
    }

    for index in 0..entries {
        queue.next().await.expect("queue ended").unwrap();
        let deadline = start + period * (index as u32);
        samples.push(
            Instant::now()
                .saturating_duration_since(deadline)
...
async fn main() {
    let iterations = env_u64("BENCH_ITERATIONS", 500).max(2);
    ...
    let queue_entries = env_u64("BENCH_QUEUE_ENTRIES", 1_000).min(u64::from(u32::MAX));
    if queue_entries >= 2 {
