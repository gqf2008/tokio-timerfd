...
mod imp {
    ...
    impl State {
        ...
        fn reset(&self) {
            ...
            *self.waker.lock().expect("timer waker mutex poisoned") = None;
        }
        ...
        fn fire(&self) {
            ...
            if let Some(waker) = self
                .waker
                .lock()
                .expect("timer waker mutex poisoned")
                .take()
            {
                waker.wake();
            }
        ...
        fn poll(&self, cx: &mut Context<'_>) -> Poll<()> {
            ...
            let mut waker = self.waker.lock().expect("timer waker mutex poisoned");
            if self.fired.swap(false, Ordering::Acquire) {
                return Poll::Ready(());
            }
            if waker
    ...
    mod tests {
        ...
        fn relative_due_time_rounds_up() {
            ...
            assert_eq!(relative_due_time(Duration::from_nanos(1)), -1);
            assert_eq!(relative_due_time(Duration::from_nanos(100)), -1);
            assert_eq!(relative_due_time(Duration::from_nanos(101)), -2);
        }
